 ...
 * the DirectDraw side lives in internal.h.
 *
...
static HRESULT AERON_DXAPI D3DViewport_QueryInterface(IDirect3DViewport* self, DxRefIid iid, void** out) {
	(void)self;
	(void)iid;
	if (out) {
		*out = NULL;
...
static uint32_t AERON_DXAPI D3DViewport_AddRef(IDirect3DViewport* self) {
	return (uint32_t)++((D3DViewportShim*)self)->refcount;
}

static uint32_t AERON_DXAPI D3DViewport_Release(IDirect3DViewport* self) {
...
static HRESULT AERON_DXAPI D3DViewport_SetViewport(IDirect3DViewport* self, D3DVIEWPORT* vp) {
	D3DViewportShim* v = (D3DViewportShim*)self;
	if (!vp) {
...
static HRESULT AERON_DXAPI D3DViewport_SetBackground(IDirect3DViewport* self, D3DMATERIALHANDLE material) {
	(void)self;
	(void)material;
	return DX_DD_OK;
}
...
static HRESULT AERON_DXAPI D3DExecBuf_QueryInterface(IDirect3DExecuteBuffer* self, DxRefIid iid, void** out) {
	(void)self;
	(void)iid;
	if (out) {
		*out = NULL;
...
static uint32_t AERON_DXAPI D3DExecBuf_AddRef(IDirect3DExecuteBuffer* self) {
	return (uint32_t)++((D3DExecBufShim*)self)->refcount;
}

static uint32_t AERON_DXAPI D3DExecBuf_Release(IDirect3DExecuteBuffer* self) {
...
static HRESULT AERON_DXAPI D3DExecBuf_Lock(IDirect3DExecuteBuffer* self, D3DEXECUTEBUFFERDESC* desc) {
	D3DExecBufShim* b = (D3DExecBufShim*)self;
	if (!desc) {
		return DX_E_INVALIDARG;
	}
...
static HRESULT AERON_DXAPI D3DExecBuf_Unlock(IDirect3DExecuteBuffer* self) {
	(void)self;
	return DX_DD_OK;
}

static HRESULT AERON_DXAPI D3DExecBuf_SetExecuteData(IDirect3DExecuteBuffer* self, D3DEXECUTEDATA* data) {
	D3DExecBufShim* b = (D3DExecBufShim*)self;
	if (!data) {
		return DX_E_INVALIDARG;
	}
...
static HRESULT AERON_DXAPI D3DTexture_QueryInterface(IDirect3DTexture* self, DxRefIid iid, void** out) {
	(void)self;
	(void)iid;
	if (out) {
		*out = NULL;
...
static uint32_t AERON_DXAPI D3DTexture_AddRef(IDirect3DTexture* self) {
	return (uint32_t)++((D3DTextureShim*)self)->refcount;
}

static uint32_t AERON_DXAPI D3DTexture_Release(IDirect3DTexture* self) {
	...
	if (pf->dwFlags & DDPF_PALETTEINDEXED8) {
		for (y = 0; y < src->height; ++y) {
			...
			for (x = 0; x < src->width; ++x) {
				...
				out[o + 3] = src->has_colorkey && (uint32_t)idx == src->colorkey ? 0 : 255;
			}
		}
	} else {
		for (y = 0; y < src->height; ++y) {
...
static HRESULT AERON_DXAPI D3DTexture_Load(IDirect3DTexture* self, IDirect3DTexture* source) {
	...
	if (dest->device && !D3DCompat_FlushSceneSegment(dest->device)) {
		return DX_E_FAIL;
	}
	...
	if (dest->device && dest->handle > 0 && dest->handle < dest->device->handle_count) {
...
static int D3DCompat_EnsureShaders(void) {
	if (!g_d3dVertexShader) {
		...
		if (!g_d3dVertexShader) {
			Aeron_LogError("aeron.dx5", "Failed to load DX5 projected triangle vertex shader");
			return 0;
		}
	}
	if (!g_d3dFragmentShader) {
		...
		if (!g_d3dFragmentShader) {
			Aeron_LogError("aeron.dx5", "Failed to load DX5 projected triangle fragment shader");
			return 0;
		}
	}
	return 1;
...
static void D3DCompat_ApplyRenderState(D3DRenderState* rs, uint32_t* texture_handle, uint32_t token,
	...
	switch (token) {
		...
		case D3DRENDERSTATE_FOGENABLE:
			if (value) {
				Aeron_LogWarn("aeron.dx5", "std3D fog is not implemented yet");
			}
			break;
		...
		case D3DRENDERSTATE_FOGTABLEEND:
			break;
		default:
			Aeron_LogWarn("aeron.dx5", "Unexpected D3DRENDERSTATE token %u in execute buffer", token);
			break;
	}
}
	...
	while (ip + sizeof(D3DINSTRUCTION) <= ip_end && !exited) {
		...
		switch (insn->bOpcode) {
			...
			case D3DOP_EXIT:
				...
				break;
			default:
				Aeron_LogWarn("aeron.dx5", "Unexpected execute-buffer opcode %u", insn->bOpcode);
				break;
		}
...
static int D3DCompat_ExecuteBuffer(D3DDeviceShim* d, D3DExecBufShim* buf) {
	...
	if (buf->vertex_count == 0 || AeronDx5_IsClassicFlightRenderingSuppressed()) {
...
static HRESULT AERON_DXAPI D3DDevice_QueryInterface(IDirect3DDevice* self, DxRefIid iid, void** out) {
	(void)self;
	(void)iid;
	if (out) {
		*out = NULL;
...
static uint32_t AERON_DXAPI D3DDevice_AddRef(IDirect3DDevice* self) {
	return (uint32_t)++((D3DDeviceShim*)self)->refcount;
}

static uint32_t AERON_DXAPI D3DDevice_Release(IDirect3DDevice* self) {
...
static HRESULT AERON_DXAPI D3DDevice_GetCaps(IDirect3DDevice* self, void* hw, void* hel) {
	(void)self;
	(void)hw;
	(void)hel;
	return DX_DD_OK;
...
static HRESULT AERON_DXAPI D3DDevice_DeleteViewport(IDirect3DDevice* self, IDirect3DViewport* viewport) {
	D3DDeviceShim* d = (D3DDeviceShim*)self;
 ...
 * @0x5995F0). */
typedef HRESULT(AERON_DXAPI* D3DEnumTextureFormatsCb)(DDSURFACEDESC*, void*);
...
static HRESULT AERON_DXAPI D3DDevice_EnumTextureFormats(IDirect3DDevice* self, void* callback, void* ctx) {
...
static int D3DCompat_FlushSceneSegment(D3DDeviceShim* d) {
	...
	if (!D3DCompat_UploadPendingDepth(d, command_buffer)) {
		Aeron_CancelCommandBuffer(command_buffer);
		Aeron_RequestFatalRendererError("Direct3D depth-surface upload");
		return 0;
	}
...
static HRESULT AERON_DXAPI D3DDevice_BeginScene(IDirect3DDevice* self) {
	D3DDeviceShim* d = (D3DDeviceShim*)self;
...
static HRESULT AERON_DXAPI D3DDevice_EndScene(IDirect3DDevice* self) {
	D3DDeviceShim* d = (D3DDeviceShim*)self;
	if (!d->scene_active) {
...
static HRESULT AERON_DXAPI D3D_QueryInterface(IDirect3D* self, DxRefIid iid, void** out) {
	(void)self;
	(void)iid;
	if (out) {
		*out = NULL;
...
static uint32_t AERON_DXAPI D3D_AddRef(IDirect3D* self) { return (uint32_t)++((D3DShim*)self)->refcount; }

static uint32_t AERON_DXAPI D3D_Release(IDirect3D* self) {
...
static HRESULT AERON_DXAPI D3D_CreateViewport(IDirect3D* self, IDirect3DViewport** out, void* outer) {
	D3DViewportShim* v;
	(void)self;
	(void)outer;
...
/* --- shim entry points (called from ddraw.c QueryInterface) -------------- */
...
void D3DCompat_Shutdown(void) {
	while (g_d3dPipelineCache) {
		D3DPipelineCache* next = g_d3dPipelineCache->next;
		Aeron_DestroyGraphicsPipeline(g_d3dPipelineCache->pipeline);
		free(g_d3dPipelineCache);
		g_d3dPipelineCache = next;
	}
	while (g_d3dSamplerCache) {
		D3DSamplerCache* next = g_d3dSamplerCache->next;
		Aeron_DestroySampler(g_d3dSamplerCache->sampler);
		free(g_d3dSamplerCache);
		g_d3dSamplerCache = next;
	}
	if (g_d3dVertexShader)
		Aeron_DestroyShader(g_d3dVertexShader);
	if (g_d3dFragmentShader)
		Aeron_DestroyShader(g_d3dFragmentShader);
	if (g_d3dWhiteTexture)
		Aeron_DestroyTexture(g_d3dWhiteTexture);
